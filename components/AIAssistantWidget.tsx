...
export const AIAssistantWidget: React.FC<AIAssistantWidgetProps> = ({ isOpen, onToggle, initialTab = 'chat' }) => {
  ...
  useEffect(() => {
    if (isOpen && activeTab === 'chat' && !isExpanded) {
      setTimeout(() => inputRef.current?.focus(), 300);
    }
  }, [isOpen, activeTab, isExpanded]);

  const handleClearChat = () => {
    if (window.confirm("Are you sure you want to clear this conversation?")) {
        const initialMsg: Message = { role: 'model', text: 'Hello, thank you for calling Patrick Law Group. My name is Hannah, and I’m your AI legal assistant on a recorded line. I’m here to learn about your situation and help connect you with the right attorney for a FREE consultation. How can I assist you today?' };
        ...
        <div className={containerClasses}>
          <div className="bg-navy-900 p-4 shrink-0 flex justify-between items-center text-white">
            <div className="flex items-center gap-3">
              <div className="bg-accent-500 p-2 rounded-full shadow-lg">
                <Bot size={20} className="text-white" />
              </div>
              <div>
                <h3 className="font-serif font-semibold leading-tight text-lg">Hannah</h3>
            ...
            <div className="flex items-center gap-1">
                <button onClick={handleToggleExpand} className="p-2 text-slate-400 hover:text-white hover:bg-navy-800 rounded-full transition hidden md:block">
                    {isExpanded ? <Minimize2 size={18} /> : <Maximize2 size={18} />}
                </button>
                <button onClick={handleClearChat} className="p-2 text-slate-400 hover:text-white hover:bg-navy-800 rounded-full transition">
                    <Trash2 size={18} />
                </button>
          ...
          <div className="bg-navy-800 px-4 py-1 flex justify-center border-t border-white/5">
             <span className="text-[10px] text-accent-200 uppercase tracking-wide font-medium flex items-center gap-1">
                 <ShieldCheck size={10} /> Educational guidance only
             </span>
          </div>
          ...
          <div className="flex-1 bg-slate-50/50 overflow-hidden relative flex flex-col">
              ...
              <div className="h-full flex flex-col">
                  ...
                  {chatMessages.map((msg, idx) => (
                    <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                      <div className={`flex gap-2 max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                         <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm mt-1 ${msg.role === 'model' ? 'bg-accent-100 text-accent-700' : 'bg-navy-100 text-navy-700'}`}>
                ...
                <div className="p-4 bg-white border-t border-slate-100">
                  <div className="flex gap-2 relative">
                    ...
                    />
                    <button onClick={() => handleSendMessage()} disabled={!inputText.trim() || isTyping} className="absolute right-1 top-1 bottom-1 aspect-square bg-navy-900 text-white rounded-lg hover:bg-accent-600 transition-all flex items-center justify-center">
                      <Send size={18} />
                    </button>
                  </div>
                </div>
            ...
            {activeTab === 'voice' && (
              <div className="h-full flex flex-col items-center justify-center p-6 text-center relative bg-gradient-to-b from-slate-50 to-white">
                {!isConnected ? (
                  ...
                  <div className="flex flex-col items-center justify-between h-full w-full py-4">
                    <div className="relative w-full flex-1 flex items-center justify-center">
                       <div className={`w-32 h-32 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 relative z-10 border-4 border-white ${isMuted ? 'bg-slate-100' : isSpeaking ? 'bg-gradient-to-br from-accent-500 to-accent-600 scale-110 shadow-accent-500/50' : 'bg-navy-900'}`}>
                    ...
                    <div className="flex items-center gap-6 mb-6">
                        <button onClick={toggleMute} className={`p-4 rounded-full border shadow-sm transition-all ${isMuted ? 'bg-red-50 text-red-500 border-red-200' : 'bg-white text-slate-600 hover:bg-slate-50 border-slate-200'}`}>
                            {isMuted ? <MicOff size={24} /> : <Mic size={24} />}
                        </button>
                        <button onClick={onToggle} className="px-8 py-4 bg-red-500 text-white rounded-full hover:bg-red-600 font-bold flex items-center gap-2 shadow-lg hover:scale-105 active:scale-95 transition-all">
                            <X size={18} /> End Call
                        </button>
                    </div>
                  </div>
                ...
                {voiceError && (
                    <div className="absolute bottom-4 left-4 right-4 bg-red-50 text-red-600 text-xs p-3 rounded-lg border border-red-100 animate-in fade-in">
                        {voiceError}
                    </div>
                )}
              </div>
